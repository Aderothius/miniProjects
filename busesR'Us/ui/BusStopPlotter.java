...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        for (Stop s : StopManager.getInstance()) {
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) {
                ...
                currentMarker.setRelatedObject(s);
                currentMarker.setIcon(stopIconDrawable);
                currentMarker.setInfoWindow(stopInfoWindow);
                currentMarker.setTitle("Stop number: " + s.getNumber() + ", " + "Stop name: " + s.getName());
                currentMarker.setPosition(new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude()));

                setMarker(s, currentMarker);
                stopClusterer.add(currentMarker);

                if (currentLocation != null) {
                    Stop stop = StopManager.getInstance().findNearestTo(new LatLon(currentLocation.getLatitude(),
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if (nearest != null) {
            // check if nearest has marker already made
            ...
            if (m != null) {
                ...
                m.setTitle(nearest.getNumber() + " " + nearest.getName());
                m.setPosition(new GeoPoint(nearest.getLocn().getLatitude(),
